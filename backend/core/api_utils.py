...
def cache_api_response(timeout: int = 300, key_prefix: str = "api", vary_on: Optional[list] = None):
    """
...
class APIResponse:
    ...
    @staticmethod
    def success(
        data: Any = None, message: str = "", status_code: int = status.HTTP_200_OK, metadata: Optional[Dict] = None
    ) -> Response:
...
def api_error_handler(func: F) -> F:
    ...
    def wrapper(*args, **kwargs):
        try:
            ...
            return APIResponse.error(
                "An unexpected error occurred. Please try again.", "INTERNAL_ERROR", status_code=status_code
            )
...
def gzip_response(min_size: int = 1024):
    ...
    def decorator(func: F) -> F:
        ...
        def wrapper(request, *args, **kwargs):
            ...
            if hasattr(response, "data") and len(json.dumps(response.data)) > min_size:
                response["Content-Encoding"] = "gzip"
