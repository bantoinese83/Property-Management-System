...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("action", choices=["create", "list", "delete", "run"], help="Action to perform")
        parser.add_argument("--name", type=str, help="Schedule name (required for create, delete)")
        parser.add_argument(
            "--frequency",
            type=str,
        ...
        parser.add_argument("--hour", type=int, help="Hour for daily/weekly/monthly backups (0-23)")
        parser.add_argument(
            "--day-of-week", type=int, choices=range(7), help="Day of week for weekly backups (0=Monday, 6=Sunday)"
        )
        parser.add_argument(
            "--day-of-month", type=int, choices=range(1, 32), help="Day of month for monthly backups (1-31)"
        )
        parser.add_argument("--retention", type=int, default=30, help="Retention period in days (default: 30)")
        parser.add_argument("--user", type=str, help="Username of the user creating the schedule")
    ...
    def _delete_schedule(self, options):
        ...
        try:
            ...
            schedule.delete()
            self.stdout.write(self.style.SUCCESS(f'Deleted backup schedule "{name}"'))
    ...
    def _run_schedules(self):
        ...
        if backups:
            self.stdout.write(self.style.SUCCESS(f"Created {len(backups)} scheduled backups"))
