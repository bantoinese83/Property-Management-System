...
def run_scheduled_backups():
    ...
    return {"backups_created": len(backups), "backup_ids": [b.id for b in backups]}
...
def create_manual_backup(name: str, backup_type: str = "full", user_id: int = None):
    ...
    backup = service.create_backup(name=name, backup_type=backup_type, user=user)

    return {"backup_id": backup.id, "status": backup.status, "file_path": backup.file_path}
