...
def check_payment_due_dates():
    ...
    upcoming_payments = RentPayment.objects.filter(due_date=three_days_from_now, status="pending")
    for payment in upcoming_payments:
        notify_rent_due(payment)

    # 2. Notify when payment becomes overdue (the day after due date)
    yesterday = today - timezone.timedelta(days=1)
    newly_overdue = RentPayment.objects.filter(due_date=yesterday, status="pending")
