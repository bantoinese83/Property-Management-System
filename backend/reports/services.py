...
class ReportService:
    ...
    def generate_financial_summary_report(
        ...
        expense_total = transactions.filter(transaction_type='expense').aggregate(
            total=Sum('amount'))['total'] or 0
        rent_collected = payments.filter(status='paid').aggregate(
            total=Sum('amount'))['total'] or 0
        ...
        return {
            ...
            'summary': {
                ...
                'profit_margin': float((income_total + rent_collected - expense_total) / max(income_total + rent_collected, 1) * 100)
            },
    ...
    def generate_property_performance_report(
        ...
        for prop in properties:
            ...
            active_leases = Lease.objects.filter(
                property_obj=prop,
                status='active',
                lease_start_date__lte=end,
                lease_end_date__gte=start
            )
            ...
            maintenance_requests = MaintenanceRequest.objects.filter(
                property_obj=prop,
                requested_date__gte=start,
                requested_date__lte=end
            )
            ...
            property_data.append({
                ...
                'roi_percentage': float((income + rent_collected - expenses) / max(prop.purchase_price or 1, 1) * 100)
            })
    ...
    def generate_tenant_report(
        ...
        return {
            ...
            'summary': {
                ...
                'average_credit_score': sum(t['credit_score'] for t in tenant_data if t['credit_score']) / max(sum(1 for t in tenant_data if t['credit_score']), 1)
            },
    ...
    def generate_maintenance_report(
        ...
        # Category breakdown
        category_stats = requests.values('category').annotate(
        ...
        while current_date <= end:
            month_requests = requests.filter(
                requested_date__year=current_date.year,
                requested_date__month=current_date.month
            )
            ...
            monthly_trends.append({
                ...
                'avg_resolution_days': month_requests.filter(
                    status='completed',
                    completed_date__isnull=False
                ).aggregate(
                    avg_days=Avg(models.F('completed_date') - models.F('requested_date'))
                )['avg_days'].days if month_requests.filter(status='completed', completed_date__isnull=False).exists() else 0
            })
        ...
        return {
            ...
            'summary': {
                ...
                'average_resolution_days': requests.filter(
                    status='completed',
                    completed_date__isnull=False
                ).aggregate(
                    avg_days=Avg(models.F('completed_date') - models.F('requested_date'))
                )['avg_days'].days if requests.filter(status='completed', completed_date__isnull=False).exists() else 0
            },
