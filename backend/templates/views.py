...
def list_templates(request):
    ...
    try:
        ...
        return Response({"error": f"Failed to list templates: {str(e)}"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
def get_template(request, template_id):
    ...
    try:
        ...
        return Response({"template": serializer.data, "variables": variables_info})

    except DocumentTemplate.DoesNotExist:
        return Response({"error": "Template not found"}, status=status.HTTP_404_NOT_FOUND)
    except Exception as e:
        return Response({"error": f"Failed to get template: {str(e)}"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
def get_template_variables(request, template_id):
    ...
    try:
        ...
        return Response(
            {"error": f"Failed to get template variables: {str(e)}"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
        )
...
def generate_document(request):
    ...
    try:
        ...
        return Response({"document": serializer.data, "message": "Document generated successfully"})

    except ValueError as e:
        return Response({"error": str(e)}, status=status.HTTP_400_BAD_REQUEST)
    except Exception as e:
        return Response(
            {"error": f"Failed to generate document: {str(e)}"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
        )
...
def download_document(request, document_id):
    ...
    try:
        document = GeneratedDocument.objects.get(
            id=document_id, created_by=request.user, status__in=["generated", "sent", "signed"]
        )

        if not document.generated_content:
            return Response({"error": "Document content not available"}, status=status.HTTP_404_NOT_FOUND)
        ...
        return Response(
            {"error": f"Failed to download document: {str(e)}"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
        )
...
def validate_template(request):
    ...
    try:
        ...
        return Response({"validation": validation_result, "is_valid": validation_result["is_valid"]})
